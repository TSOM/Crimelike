--Luxury House number 1
--AUTHOR: MENDONCA
--25 x 15 plot

defineTile('p', 'PAVEMENT')
defineTile('#', 'WALL_BRICK_DARK')
defineTile('=', 'COUNTER_WOOD_RED')
defineTile(',', 'FLOOR_WOOD_TWO')
defineTile('c', 'CONCRETE_FLOOR')
defineTile('+', 'SINK')
defineTile('D', 'DOOR_FANCY_2', 'WALL_BRICK_DARK')
defineTile('O', 'WINDOW_HOUSE', 'WALL_BRICK_DARK')
defineTile('W', 'WC')
defineTile('~', 'GRASS_ONE')
defineTile('N', 'WHB')
defineTile('H', 'SHOWER')
defineTile('C', 'CHAIR_WOODEN')
defineTile('T', 'TABLE_WOODEN')
defineTile('B', 'BED_WOODEN_E')
defineTile('b', 'BED_WOODEN_W')
defineTile('E', 'BED_WOODEN_N')
defineTile('e', 'BED_WOODEN_S')
defineTile('/', 'SHELF_WOOD')
defineTile('%', 'SHELF_METAL')
defineTile('x', 'TELEVISION', 'FLOOR_WOOD_TWO')
defineTile('X', 'TELEVISION', 'FLOOR_WOOD_TWO')
defineTile('F', 'FRIDGE', 'FLOOR_WOOD_TWO')
defineTile('S', 'SOFA_RIGHT_1', 'FLOOR_WOOD_TWO')
defineTile('s', 'SOFA_RIGHT_2', 'FLOOR_WOOD_TWO')
defineTile('<', 'STAIR_WOODEN_UP')
defineTile('>', 'STAIR_WOODEN_DOWN')
defineTile('A', 'BATH_E')
defineTile('a', 'BATH_W')
defineTile('G', 'DOOR_GARAGE')
defineTile('u', 'DESK_WOODEN', 'FLOOR_WOOD_TWO')
defineTile('-', 'FENCE_IRON', 'GRASS_ONE')
defineTile('t', 'LAMP_STANDARD', 'FLOOR_WOOD_TWO')
defineTile('v', 'PLANTPOT_FIXED')
defineTile('g', 'GATE_IRON', 'GRASS_ONE')
defineTile('K', 'POOL_TABLE_E')
defineTile('k', 'POOL_TABLE_W')
defineTile('M', 'HIFI', 'FLOOR_WOOD_TWO')

return{
[[ppppppppppppppppppppppppp]],
[[p-----------------gg----p]],
[[p-~~~~~~~~~~~~~~~~pp~~~-p]],
[[p-~~~~~~~~~~~~~~~~pp~~~-p]],
[[p-~~#OOOO#~~~~~~~~pp~~~-p]],
[[p-~~Ot,,vO~~~~~~~~pp~~~-p]],
[[p#OO#,,,,#OO###OO#GG##O#p]],
[[pO,,,,,,,,,,,#t,,#cc%#uOp]],
[[p#S,,,,,,=,,F#,,,Dcc%#,Dp]],
[[pOs,,,,X,=,,,D,,,#cc%#,Op]],
[[p#,,,,,,,=,,+#,,v#####,#p]],
[[p#OO#,,,,#OO##,,,,,,,,,Op]],
[[ppppOv,,tOpppO<>##O##O##p]],
[[~~~p######ppp####pppppppp]],
[[~~~ppppppppppppppp~~~~~~~]]}

--Basement:

--return
--[[]]
--[[]]
--[[]]
--[[]]
--[[]]
--[[]]
--[[             #########]]
--[[             #M,,,,,/#]]
--[[             #,,,,,,/#]]
--[[             #,,Kk,,/#]]
--[[             #,,,,,,/#]]
--[[             #,,,,,,/#]]
--[[             #,<######]]
--[[             ####]]

--First Floor:

--return
--[[]]
--[[]]
--[[]]
--[[]]
--[[    #OOOO#]]
--[[    Ot,,vO]]
--[[ #OO#,,,,#OO##OO#OO##O#]]
--[[ O,,,,,,,SS//#u,v#AaWNvO]]
--[[ #E,,,,,,,,,,#C,,D,,,,,#]]
--[[ Oe,,,,,,,,,,D,,########]]
--[[ #,,,,,,,,,,,#,,D,,,,,X#]]
--[[ #OO#,,,C#OO##,,#,,Bb,tO]]
--[[    Ov,,uO   O><##O##O##]]
--[[    ######   ####]]

--Second Floor:

--return
--[[]]
--[[]]
--[[]]
--[[]]
--[[]]
--[[]]
--[[             ###OO#OO###]]
--[[             O,,,,M,,,,O]]
--[[             #S,,,,,,,,O]]
--[[             #S,,,,,,,,O]]
--[[             #,,,,,,,,,O]]
--[[             #,,,X,//,,O]]
--[[             O,>##O##O##]]
--[[             ####]]
