--Motel number 1
--AUTHOR: MENDONCA
--35 x 20 plot

defineTile('p', 'PAVEMENT')
defineTile('8', 'WALL_BRICK_DARK')
defineTile('#', 'WALL_DRY')
defineTile('=', 'COUNTER_WOOD_RED')
defineTile(',', 'FLOOR_WOOD_TWO')
defineTile('.', 'FLOOR_TILE_YORKSTONE')
defineTile(';', 'CARPET_GREY')
defineTile('c', 'CONCRETE_FLOOR')
defineTile('x', 'CONCRETE_CRACK_1')
defineTile('+', 'SINK')
defineTile('D', 'WALL_BRICK_DARK_DOOR')
defineTile('d', 'WALL_DRY', 'DOOR_APART_1')
defineTile('O', 'WINDOW_HOUSE', 'WALL_BRICK_DARK')
defineTile('W', 'WC')
defineTile('~', 'GRASS_ONE')
defineTile('N', 'WHB')
defineTile('H', 'SHOWER')
defineTile('C', 'CARPET_GREY', 'CHAIR_WOODEN')
defineTile('T', 'GRASS_ONE', 'TREE_SMALL')
defineTile('B', 'CARPET_GREY', 'BED_WOODEN_W')
defineTile('b', 'CARPET_GREY', 'BED_WOODEN_E')
defineTile('E', 'CARPET_GREY', 'BED_WOODEN_N')
defineTile('e', 'CARPET_GREY', 'BED_WOODEN_S')
defineTile('/', 'SHELF_WOOD')
defineTile('%', 'SHELF_METAL')
defineTile('X', 'TELEVISION')
defineTile('F', 'FLOOR_WOOD_TWO', 'FRIDGE')
defineTile('u', 'CARPET_GREY', 'DESK_WOODEN')
defineTile('?', 'COUNTER_WOOD_RED', 'CASH_REGISTER')
defineTile('"', 'SAFE_SIMPLE')

return{
[[ppppppppppppppppppppppppppppppppppp]],
[[p88888888888OO888OO888OO888OO88~~~p]],
[[p8Bb;#HW#+#;;;E#E;;;#;;;E#E;;;8~~~p]],
[[pO;;;#.N#.#uC;e#e;Cu#uC;e#e;Cu8~~Tp]],
[[pO;C;#d##d#;;;;#;;;;#;;;;#;;;;8~~~p]],
[[p8;u;;;;d,#;#######;#;#######;8~~~p]],
[[p8#######,#;d.N#N.d;#;d.N#N.d;8T~~p]],
[[p8Bb;#HW#,#;#WH#HW#;#;#WH#HW#;8~~~p]],
[[pO;;;#.N#,#d#######d#d#######d8~~~p]],
[[pO;C;#d##,,,,,,,,,,,,,,,,,,,,,O~T~p]],
[[p8;u;;;;d,88OO888OO888OO888OO88~~~p]],
[[p88888888D8~~~T~~~~~T~~~~~~~~T~~~~p]],
[[pccccccccccccccxccccccccccccccccccp]],
[[p88O8888O88cccccccccccccccccccccccp]],
[[p8WN.H#,,,Dcc~~~~cccccccccxcccxcccp]],
[[p8##d##,?=8cc~TT~ccccccxccccccccccp]],
[[pOBb;X#,,+Occ~~~~ccccccccccccxccccp]],
[[p8";;;d,,F8ccccccccccccxccccccccccp]],
[[p88O8O88O88cccccccccccccccccccccccp]],
[[ppppppppppppppppppppppppppppppppppp]]}
