	...
	for stw = 1, st.sizew do for sth = 1, st.sizeh do if stw == 1 or stw == st.sizew or sth == 1 or sth == st.sizeh then
		...
		for _, dt in ipairs(self.tiles) do if dt.type == type then
			...
			for dtw = 1, dt.sizew do for dth = 1, dt.sizeh do if dtw == 1 or dtw == dt.sizew or dth == 1 or dth == dt.sizeh then
				...
				local okk
					for i = 1, self.block.w do
						local ret, aok = self:matchTile(st[i+stwr][1], dt[i+dtwr][self.block.h*dt.sizeh])
						allok = allok or aok
						if not ret then okk = false end
					end
					for i = 1, self.block.w do
						local ret, aok = self:matchTile(st[i+stwr][self.block.h*st.sizeh], dt[i+dtwr][1])
						allok = allok or aok
						if not ret then okk = false end
					end
					for j = 1, self.block.h do
						local ret, aok = self:matchTile(st[1][j+sthr], dt[self.block.w*dt.sizew][j+dthr])
						allok = allok or aok
						if not ret then okk = false end
					end
					for j = 1, self.block.h do
						local ret, aok = self:matchTile(st[self.block.w*st.sizew][j+sthr], dt[1][j+dthr])
						allok = allok or aok
						if not ret then okk = false end
					end
